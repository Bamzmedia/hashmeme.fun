            ...
            <div className="relative min-h-[400px]">
                {loading ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center animate-pulse">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-400">Syncing Mirror Node...</span>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                        {tokens.length > 0 ? (
                                ...
                                <div key={token.token_id || index} className="group relative bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] p-8 hover:border-indigo-500/30 transition-all duration-500 shadow-2xl overflow-hidden">
                                        ...
                                        <div className="flex space-x-6">
                                            ...
                                            <div className="flex flex-col justify-center">
                                                <div className="flex items-center space-x-2 mb-2">
                                                    ...
                                                    {filter === 'all' && index < 3 && (
                                                        <span className="text-[10px] font-black bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded border border-emerald-500/20 uppercase tracking-tighter">NEW</span>
                                                    )}
                                                    {filter === 'hot' && index < 3 && (
                                                        <span className="text-[10px] font-black bg-pink-500/10 text-pink-400 px-2 py-0.5 rounded border border-pink-500/20 uppercase tracking-tighter">HOT</span>
                                                    )}
                                                </div>
                                    ...
                                    <div className="relative z-10 grid grid-cols-2 gap-4 mt-8 pt-8 border-t border-white/5">
                                        <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                                            <div className="text-[10px] text-gray-500 font-black uppercase tracking-widest mb-1">Total Supply</div>
                            ...
                            <div className="col-span-full py-20 text-center text-gray-600 font-black uppercase tracking-widest border-2 border-dashed border-white/5 rounded-[3rem]">
                                No Memes Found Tracking the Network...
                            </div>
                        )}
                    </div>
                )}
