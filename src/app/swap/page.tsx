...
export default function SwapPage() {
    ...
    const executeSwap = async () => {
        ...
        setStatus("Requesting wallet approval for Blue Bridge...");
            ...
            <nav className="fixed top-0 left-0 w-full h-20 border-b border-white/5 bg-black/40 backdrop-blur-md z-50 px-8 flex items-center justify-between">
                <div className="flex items-center space-x-12">
                    <Link href="/" className="flex items-center space-x-3 group">
                        ...
                        <span className="text-lg font-black tracking-tighter uppercase">HashMeme / SWAP</span>
                    </Link>
                    ...
                    <div className="hidden md:flex items-center space-x-8">
                        ...
                        <Link href="/stake" className="text-[10px] font-bold text-white/40 hover:text-white uppercase tracking-widest transition-colors">Liquidity</Link>
                    </div>
                </div>
                    ...
                    {showSettings && (
                        ...
                        >
                            ...
                            <input 
                                ...
                                className="w-full bg-white/5 border border-white/10 px-4 py-3 text-xs font-mono mb-6 focus:border-blue-500 outline-none transition-all"
                            />
                            <div className="flex justify-between items-center">
                                <span className="text-[10px] font-bold uppercase text-white/30 tracking-widest">Slippage</span>
                                <div className="flex space-x-2">
                                    {[0.5, 1, 3].map(val => (
                                        <button key={val} onClick={() => setSlippage(val)} className={`px-4 py-1.5 text-[9px] font-black border transition-all ${slippage === val ? 'border-blue-500 bg-blue-500/20 text-white' : 'border-white/10 text-white/30'}`}>{val}%</button>
                                    ))}
                                </div>
                            </div>
                        </motion.div>
                        ...
                        <div className="p-6 bg-white/5 border border-white/5 hover:border-blue-500/30 transition-all focus-within:border-blue-500 focus-within:shadow-neon-blue">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <div className="flex items-center space-x-4">
                                    ...
                                    <div className="px-5 py-2 bg-white text-black font-black text-xs">HBAR</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-6 bg-white/5 border border-white/5">
                            ...
                            <div className="flex justify-between items-center">
                                <input 
                                    ...
                                    className="bg-transparent text-5xl font-black w-full outline-none placeholder:text-white/5 tracking-tighter"
                                />
                                <div className="px-5 py-2 border border-white/10 text-white/40 font-black text-xs">TOKEN</div>
                            </div>
                        </div>
                    </div>
